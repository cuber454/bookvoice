...
class CatalogActivity(private val act: SectionActivity) {
     ...
     *
     *  Место в каталоге (msg2445) здесь НЕ стираем (#23, msg4247). Окно
     *  закрывается не только при полном выходе, но и при обычном уходе на полку
     *  (кнопка «Библиотека», «назад» из корня) — и именно тогда запомненное место
     *  должно пережить закрытие. Раньше память чистилась здесь, поэтому
     *  сохранённая позиция умирала в тот же миг, и следующий вход в «Сетевые
     *  библиотеки» начинался с корня, хотя место было записано.
     *
     *  Полного выхода это не касается: память живёт в процессе, а «Выход из
