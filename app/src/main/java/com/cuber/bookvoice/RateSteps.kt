...
object RateSteps {
    ...
    /** 0.5 … 6.0 ровным шагом 0.1 — 56 значений.
     *
     *  msg5979: потолок 4.0 поднят до 6.0 «и потом протестируем». Смысл именно
     *  в проверке: наш ползунок ничего не зажимает, значение уходит движку
     *  (`SpeechPlayer.speed` → `setSpeechRate`), а выше двух у каждого движка
     *  свой настоящий предел — заказ 6.0 он вправе проигнорировать. Где предел
     *  на самом деле, видно по размеру файлов в журнале (см. `synthFinished`). */
    val SPEED: List<Float> = tenths(0.5f, 6.0f)
