...
class SentenceAdapter(
     ...
     *
     *  0.4.81: тут была ошибка поиска, из-за которой лента не ехала за чтением.
     *  Строки главы идут по возрастанию `first`: сначала строка-заголовок
     *  (`first = -1`), потом абзацы (`first = 0, 3, 7…`). Поиск же считал
     *  заголовок «строкой дальше» (`first` не попадал в `0..s`) и уходил ВЛЕВО —
     *  то есть от нужного абзаца. Пока в главе было три и больше строк, середина
     *  попадала на абзац и ошибка не проявлялась; а если у главы ровно два ряда
     *  (заголовок и один абзац — так устроена каждая страница книги-скана),
     *  поиск всегда возвращал -1: текст стоял, чтение уходило вперёд. Сравниваем
     *  по-настоящему: `first <= s` — тогда идём вправо, иначе влево. */
    fun flatOf(chapter: Int, sentence: Int): Int {
        ...
        // Строку-заголовок не отдаём: предложений у неё нет (если абзацев в
        // главе не нашлось, последней подходящей осталась она — значит строки нет).
        if (best >= 0 && rows[best].first < 0) return -1
        return best
    }
