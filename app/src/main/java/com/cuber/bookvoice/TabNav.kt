...
object TabNav {
    ...
    fun focusHeader(v: View, delayMs: Long = 550) {
        ...
        // msg2302: НЕ ретраить перенос. TalkBack объявляет КАЖДОЕ app-событие
        // переноса, но флаг isAccessibilityFocused() при этом не выставляет
        // синхронно — проверка «не подтверждён» всегда считала перенос
        // неудавшимся, и заголовок озвучивался по разу на событие (2-3 дубля
        // в 0.4.6). Однократная отправка = одно объявление (урок msg1629/1647).
        // Повтор не нужен и потому, что сам заголовок теперь кандидат фокуса с
        // первого кадра (heading+screenReaderFocusable) — TalkBack озвучивает
        // его и при авто-парковке.
    }
