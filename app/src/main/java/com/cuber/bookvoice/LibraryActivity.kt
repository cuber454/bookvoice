...
class LibraryActivity(private val act: SectionActivity) {
    ...
    private fun showMoreMenu() {
        ...
        // На «Все» пункт теперь есть тоже (Сергей: «добавляй на все, пускай
        // предупреждает просто») — здесь он чистит всю полку, и предупреждение с
        // числом книг тут особенно к месту. Нет пункта только на пустой вкладке:
        // удалять нечего.
        val clearCount = shownRecords.size
    ...
    private fun removeBook(rec: BookRecord, withFiles: Boolean): Boolean {
        ...
        // Синхронизация (msg6142): «здесь книги нет» само по себе между
        // устройствами не едет — оставляем метку удаления, по ней книга уйдёт с
        // полки второго устройства. Синхронизация выключена — метка не ставится.
        SyncStore.recordDelete(act, rec.name)
